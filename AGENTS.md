...
## Що це за проект
Платформа AI-агентів на базі Multi-Model Codex. Мета — SaaS (місячна оренда per agent).
...
## AI Моделі
- **Codex-sonnet-4-6** — основна логіка, генерація, суддя
- **Codex-haiku-4-5-20251001** — класифікація, швидкий JSON, memory summary
...
| 9 | Sales Telegram Import | `agents/sales_telegram/` | ✅ Telethon whitelist import для живих діалогів менеджера |
...
- `agents/sales/human_trainer.py` — аналізує живі менеджерські Telegram-діалоги з `source='tg_sales_human'`
- `agents/sales_telegram/` — Telethon імпорт приватних клієнтських чатів менеджера до `2024-05-15`
- `agents/instagram/instagram_agent.py` — обробка Instagram DM
- `agents/instagram/file_handler.py` — фото/PDF через Codex Vision
...
Sources:
- `instagram` — Instagram Sales Agent / SendPulse.
- `telegram` — службовий Telegram бот/менеджерський інтерфейс.
- `tg_sales_human` — живі приватні Telegram-діалоги менеджера з клієнтами. Тільки whitelist, тільки `agents/sales_telegram/`, тренувати через `agents/sales/human_trainer.py`.
...
**JSON Parsing** — Codex іноді додає текст навколо JSON. Використовуй `raw.find("[")` або Pydantic.
...
## Фіналізація сесії
`.Codex/commands/finalize.md` — аналізує розмову і дописує нові записи до `BUGS_AND_DECISIONS.md`.
...
### Живі Telegram-діалоги менеджера не є службовим Telegram
Особистий Telegram менеджера містить еталонні sales-відповіді, але `source='telegram'` уже використовується службовим ботом і раніше ламав trainer чужими темами.
**Правило:** людські sales-діалоги імпортувати тільки як `source='tg_sales_human'` через whitelist у `agents/sales_telegram/`; аналізувати тільки `agents/sales/human_trainer.py`, не основним `trainer.py`.
