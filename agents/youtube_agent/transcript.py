...
def get_video_id(url: str) -> str:
    """
    Витягує video_id з будь-якого формату YouTube URL.

    Підтримує youtube.com/watch?v=..., youtu.be/..., shorts/...

    Raises:
        ValueError: якщо video_id не знайдено
    """
    ...
    if "shorts" in parts:
        ...
        if idx + 1 < len(parts) and parts[idx + 1]:
            return parts[idx + 1]
    raise ValueError(f"Не вдалося знайти video_id в URL: {url}")
